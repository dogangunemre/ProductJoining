...
{
    public class OzdilekteyimJoinMarul
    {
        public void Join()
        {
            ...
            var contextM = new ProductContextMarul();

            List<Product> BazaaramedPro = contextM.Products.Where(s => s.State == true && !string.IsNullOrWhiteSpace(s.Barcode)).ToList();
            foreach (var item in BazaaramedPro)
            {
                ...
                if (product == null)
                {
                    ...
                    product1.Source = 1;
                    int? catID = contextM.Categories.FirstOrDefault(s => s.ID == item.CategoryID).FakeID;
                    product1.CategoryID = contextO.Categories.FirstOrDefault(s => s.ID == catID).ID;

                    Brand brandBazaaramed = contextM.Brands.FirstOrDefault(b => b.ID == item.BrandID);
                    ...
                    File file = contextM.Files.FirstOrDefault(s => s.ProductID == item.ID);
                    if (file == null)
                    {
